...
class UserController extends Controller
{
    ...
    public function setting()
    {
        ...
        $phonecodes = Country::where('active', 1)->where('phonecode', '<>', 0)->orderBy(
            'phonecode',
            'asc'
        )->select('phonecode')->distinct()->get()->all();

        if (1 === $user->user_type) {
            return view('panel.company.setting', [
                'user' => $user, 'cityname' => $city, 'countryname' => $country, 'countries' => $countries,
    ...
    public function verify(Request $request)
    {
        if ('verifyEmail' === $request->input('key')) {
            return response()->json([
                'status' => User::where('email', $request->input('value'))->where(
                    'id',
                    '<>',
                    $request->user()->id
                )->exists(),
        ...
        if ('verifyUsername' === $request->input('key')) {
            return response()->json([
                'status' => User::where('username', $request->input('value'))->where(
                    'id',
                    '<>',
                    $request->user()->id
                )->exists(),
