@extends('layouts.app', ['ACTIVE_TITLE' => 'CHAT', 'ACTIVE_CHAT_ROUTES' => true, 'ACTIVE_PAGE' => 'search', 'CHATTING_ROUTE' => url('') . '/messages/' . auth()->user()->id . '_'])
...
<div class="main-bg d-flex pb-0">
  <div class="row flex-column m-0 p-0 w-100 chat-section">
...
<script type="text/javascript">
  ...
  $('#userSearchIcon').click(function () {
    if($("#userSearchKey").val() == '') {
      ...
      $.ajax({
        url: '{{ route("messages.search.filter") }}',
        method: "POST",
        data: options,
        success: function(res) {
          if (res.length) {
            ...
            var html = '<div class="d-flex justify-content-center align-items-center no-members">' +
              '<div class="text-center"><div>NO CHAT FOUND</div>' +
              '<div>Add a buddy to start chat</div></div>' +
              '</div>';
            $('.member-body').html(html);
            $('.member-body').show();
          }
