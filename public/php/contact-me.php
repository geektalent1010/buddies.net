...
if ($_POST) {
    ...
    if ( ! isset($_SERVER['HTTP_X_REQUESTED_WITH']) && 'xmlhttprequest' !== mb_strtolower($_SERVER['HTTP_X_REQUESTED_WITH'])) {
    ...
    if ( ! isset($_POST['userChecking']) || ! isset($_POST['userName']) || ! isset($_POST['userEmail']) || ! isset($_POST['userCompany']) || ! isset($_POST['userPhone']) || ! isset($_POST['userMessage'])) {
        ...
        $output = json_encode(['type' => 'error', 'text' => '<i class="icon ion-close-round"></i> Input fields are empty!']);
        exit($output);
    }
    ...
    if ( ! empty($_POST['userChecking'])) {
        $output = json_encode(['type' => 'error', 'text' => '<i class="icon ion-checkmark-round"></i> Thanks for your submission']);
        exit($output);
    }

    // PHP validation for the fields required
    if (empty($_POST['userName'])) {
        $output = json_encode(['type' => 'error', 'text' => '<i class="icon ion-close-round"></i> We are sorry but your name is too short or not specified.']);
        exit($output);
    }

    if ( ! filter_var($_POST['userEmail'], FILTER_VALIDATE_EMAIL)) {
        $output = json_encode(['type' => 'error', 'text' => '<i class="icon ion-close-round"></i> Please enter a valid email address.']);
        exit($output);
    }

    // Phone number has been set to require at least 6 characters.
    if (mb_strlen($_POST['userPhone']) < 6) {
        $output = json_encode(['type' => 'error', 'text' => '<i class="icon ion-close-round"></i> Please enter a valid phone number.']);
        exit($output);
    }

    // To avoid too small message, you can change the value of the minimum characters required. Here it's <20
    if (mb_strlen($_POST['userMessage']) < 20) {
        $output = json_encode(['type' => 'error', 'text' => '<i class="icon ion-close-round"></i> Too short message! Take your time and write a few words.']);
        exit($output);
    }
    ...
    if ( ! $Mailsending) {
        ...
        $output = json_encode(['type' => 'error', 'text' => '<i class="icon ion-close-round"></i> Oops! Looks like something went wrong, please check your PHP mail configuration.']);
        exit($output);

    }
    $output = json_encode(['type' => 'message', 'text' => '<i class="icon ion-checkmark-round"></i> <span class="name-success">Hello ' . $_POST['userName'] . '</span>, Your message has been sent, we will get back to you asap !']);
    exit($output);
