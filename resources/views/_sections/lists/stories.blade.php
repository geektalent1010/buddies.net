  ...
  @foreach ($stories as $index => $story)
    <div class="w-100 post-item mb-3">
        ...
        <div class="member-item">
          <div class="member-link">
            ...
            <div class="member-name">
              <a class="member-name-wrp" href="{{ route('profile.index', [ 'userID' => $story->created_by ]) }}">
      ...
      <div id="post{{ $index + 1 }}" class="collapse" aria-labelledby="posthead{{ $index + 1 }}" data-parent="#stories">
        @if (isset($story->featured_image_url))
        <a class="w-100" data-fancybox href="{{ asset('uploads/posts/'.$story->featured_image_url.'?'.time()) }}">
          <img class="w-100 h-auto" src="{{ asset('uploads/posts/'.$story->featured_image_url.'?'.time()) }}" alt="featured image">
        </a>
        @endif
      ...
      <div class="like-section">
        ...
        @if ($story->created_by === $authUser->id)
          <div class="option-icon-section">
            <a class="option-icon-btn" href="{{ route('story.edit.index', [ 'id' => $story->id ]) }}">
