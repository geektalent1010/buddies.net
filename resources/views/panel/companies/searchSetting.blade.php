...
@php
    $main_interests = isset($searchSetting) ? explode(",", $searchSetting->categories) : [];
@endphp
...
<script type="text/javascript">
    ...
    var interests = '{{ (isset($searchSetting) && $searchSetting->categories) ? $searchSetting->categories : '' }}';
    var interested_category = interests ? interests.split(',') : [];
    var selected_distance = '{{ (isset($searchSetting) && $searchSetting->distance) ? $searchSetting->distance : '' }}';
    ...
    $('.business-category-check').on('click', function() {
        ...
        if (interested_category.indexOf(category) < 0) {
            if (interested_category.length > 0) {
                toastr['error']('You cannot select only 1 category.', 'Alert');
                return;
            }
